class ApiFeature {
  ...
  filter() {
    ...
    excludesFields.forEach((fields) => delete queryStringObject[fields]);
  ...
  search(modelName) {
    if (this.queryString.keyword) {
      ...
      if (modelName === "ReportSchema") {
        querySearch.$or = [
          { pharmacistName: { $regex: keyword, $options: "i" } },
          { consultantName: { $regex: keyword, $options: "i" } },
        ];
      } else if (modelName === "AllDrug") {
        querySearch = { drugName: { $regex: keyword, $options: "i" } };
      } else {
        querySearch = { userName: { $regex: keyword, $options: "i" } };
      }
